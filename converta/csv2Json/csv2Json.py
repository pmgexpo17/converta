...
class Csv2Json(object):
	...
	#------------------------------------------------------------------#
	# constructor parms -
	# - request: contains the settings dict
	#------------------------------------------------------------------#
	...
	def appendValue(self, dbObjKey, value):
		...
		_list = self.db[dbObjKey]
		_list += [value]
		self.db[dbObjKey] = _list
	...
	def parse(self, request):
		...
		try:
			...
			self.db = request.sessionDb.make(workSpace, makeFlag='n')
		except Exception as exc:
			log.error('@parse, ' + str(exc))
			raise
